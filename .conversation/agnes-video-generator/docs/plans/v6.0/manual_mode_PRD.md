...
3. **对外开放**：所有中间产物的**格式、存储位置、修改方式**都规范到"外部工具可直接处理"的程度；
   提供可复制的示例 prompt，用户可将其交给本地免费 Agent（如 TraeWork、opencode、workbuddy、CodeBuddy CLI 等）处理后再回填。
...
| **外部 Agent** | 用户自行引入的、可读写本地文件与执行命令的 AI Agent（如 TraeWork、opencode、workbuddy、CodeBuddy CLI），或手动工具（ffmpeg、Python 脚本等）。 |
...
| 3 | **外部 Agent** | 用户自有 Agent | 复制 `cd <目录> && traework` + 提示词模板 → Agent 处理回填 → 已修改，继续 | 同上 |
...
| 🤝 外部 Agent | 复制 `cd <任务目录> && traework` + 协作 prompt（§七 示例1）→ Agent 处理回填 → 「已修改，继续」 | prompt 模板已限定"保持结构不变、仅增强画面描述" |
...
| 🤝 外部 Agent | 复制 `cd <任务目录> && traework` + §七 示例5 → 处理回填 → 继续 | 输出建议命名 `*_v2.png` 再上传，避免覆盖原文件前无备份 |
...
| 🤝 外部 Agent | 复制 `cd <任务目录> && traework` + §七 示例4 → 处理（输出 `video_fixed.mp4`）→ 上传/覆盖 → 继续 | Agent 勿重编码音频流以外的轨，保持画面比例 |
...
| 🤝 外部 Agent | 复制 `cd <任务目录> && traework` + 自定义 prompt → 处理回填 → 继续 | 成片为最终交付物，建议确认无上游问题时再改 |
...
- 提示文案："手动模式会在每个暂停点等待你确认或修改产物，可配合外部 AI 工具（TraeWork / opencode / workbuddy 等）使用。运行中也可随时在自动/手动间切换。"
...
3. **处理本检查点**（三卡片并行，任选其一，点开对应面板）：
   ...
   - 🤝 **外部 Agent**：展示 `cd <任务目录> && traework` 一键复制 + 协作 prompt 模板一键复制（§七，占位符已填充真实路径）→ Agent 处理回填后点「🤝 已修改，继续」
...
> （如 TraeWork、opencode、workbuddy、CodeBuddy CLI），以及手动命令行工具。
